...
//
// מאז Item 7 (עימוד בצד שרת — F8), תצוגת הרשימה (list) של ClientsPage מבצעת
// חיפוש/סינון-שלב/מיון-עמודות-בסיס ישירות מול ה-DB (.or ilike / .eq / .order),
// ולא דרך הפונקציות כאן. הפונקציות בקובץ הזה עדיין בשימוש מלא בשני נתיבים:
//   1. תצוגת קנבן (ClientsKanban) — טוענת את *כל* לקוחות הארגון (לא מעומדת,
//      כי הקנבן מציג את כל השלבים יחד) ומפעילה חיפוש/סינון/מיון כאן, בצד
//      לקוח, בדיוק כמו לפני Item 7.
//   2. תצוגת הרשימה — פעולות "עמוד-מקומי בלבד": מיון לפי עמודה שאי אפשר
//      למיין בצד שרת (contacts, status — ר' הערה ב-ClientsPage) וסינוני-עמודה
//      (matchesFilters) ממשיכים לפעול רק על 50/25/100 השורות שכבר נטענו לעמוד
//      הנוכחי, לא על כל הטבלה — ClientsPage מציג הערה מתאימה למשתמש כשזה קורה.
//   3. ייצוא CSV — מושך את כל הדפים התואמים ומפעיל matchesFilters/sortClients
//      על הסט המלא לפני הכתיבה לקובץ (כדי שהייצוא יהיה מדויק גם לסינוני-עמודה).
// buildColumns/getCellText משמשים גם את טופס "לקוח חדש" וגם את בונה הסינון.
